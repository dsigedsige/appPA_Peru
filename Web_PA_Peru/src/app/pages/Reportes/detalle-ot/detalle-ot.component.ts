...
inicializarDom(){
 ...
 getCargarCombos(){ 
    this.spinner.show();
    combineLatest([this.ordenTrabajoService.get_servicio(this.idUserGlobal), this.listaPreciosService.get_tipoOrdenTrabajo(), this.ordenTrabajoService.get_Distritos(), this.ordenTrabajoService.get_Proveedor_usuario(this.idUserGlobal), this.aprobacionOTService.get_estados() ]).subscribe( ([ _servicios, _tipoOrdenTrabajo, _distritos, _proveedor,_estados ])=>{
...
descargarFotosOT(pantalla:string){
  ...
  downloadFormat(object:any){
    ...
    if ( object.urlArchivo == '' || object.urlArchivo == null || object.urlArchivo == undefined  ){
        return;
    } 
 
    const id_link :any = document.getElementById('id_link');  
    setTimeout(function () {
        id_link.href = object.urlArchivo
